...
- 🛡️ **安全で信頼性が高い** - Electron のセキュリティベストプラクティスを使用し、非破壊のディスク修復「リセット」ボタンを提供
...
- **読み書きの説明**：
  - **基本操作**: ファイルのコピー、切り取り、削除、名前変更をサポート（メタデータレベルの操作）
  - **書き込み制限（グラフィカルソフトウェア）**: グラフィカルソフトウェア（Electron GUI 版）はカーネル書き込み権限がないため、元のファイルに対して直接「上書き」でデータを変更することはサポートされていません
  - **編集推奨**: アトミック書き込み (Atomic Write) をサポートするエディタ（VS Code / Kate など）を使用してください。これらのツールは保存時に「新規作成して古いファイルを置き換える」方式を使用するため、元の場所への上書き制限を回避できます。また、ファイルをローカルの Mac にコピーして編集し、編集後にコピーし戻すことをお勧めします
  - **補足説明**: `/ninja/kamui.sh` は元のファイルに対して直接「上書き」でデータを変更することをサポートしており、ファイルを直接編集する必要があるシナリオに適しています [^2]
...
- **システム整合性保護（SIP）**（オプション）: SIP を無効にするには、リカバリモードで操作する必要があります：
...
[^2]: [nohajc/anylinuxfs](https://github.com/nohajc/anylinuxfs) により提供され、二次カプセル化されています
