...
>  - **写入限制（图形化软件）**：图形化软件（Electron GUI 版本）由于缺乏内核写权限，不支持直接在原文件上"涂改"数据
>  - **编辑建议**：请使用支持原子写入 (Atomic Write) 的编辑器（如 VS Code / Kate）。这类工具保存时会"新建并替换"旧文件，从而绕过原位擦写限制
>  - **补充说明**：`/ninja/kamui.sh` 支持直接在原文件上"涂改"数据，适用于需要直接修改文件的场景
