...
#
# 设置语言: LANG=ja bash dev.sh (日文) 或 LANG=en bash dev.sh (英文)
################################################################################
...
if [ -f "$SCRIPT_DIR/dev-lang.sh" ]; then
	source "$SCRIPT_DIR/dev-lang.sh"
else
	# 如果找不到语言文件，使用简单的回退函数
	t() { echo "$1"; }
fi
...
fix_electron() {
    ...
    if [ -z "$ELECTRON_INSTALL_SCRIPT" ]; then
        ...
        if [ -z "$ELECTRON_INSTALL_SCRIPT" ]; then
            print_error "$(t electron_script_not_found)"
            print_info "$(t electron_manual_fix)"
            set -e
            return 1
        fi
    fi
...
install_dependencies() {
    ...
    if ! check_electron; then
        ...
        if ! fix_electron; then
            print_error "$(t electron_fix_failed)"
            print_info "$(t electron_manual_fix_alt)"
            exit 1
        fi
    else
...
run_app() {
    ...
    if ! check_electron; then
        ...
        if ! fix_electron; then
            print_error "$(t electron_fix_failed)"
            echo ""
            print_info "$(t electron_manual_fix_simple)"
