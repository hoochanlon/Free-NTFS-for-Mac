	...
	case "$SCRIPT_LANG" in
		zh)
			case "$key" in
				...
				electron_clean_install) echo "建议清理后重新安装：" ;;
				...
				app_starting_dev) echo "启动 Electron 应用（开发模式）..." ;;

				# 文件检查相关
				files_checking) echo "检查必要文件..." ;;
				files_not_found) echo "找不到文件: $2" ;;
				files_check_success) echo "必要文件检查完成" ;;
				files_setting_permissions) echo "设置脚本执行权限..." ;;
				files_checking_dirs) echo "检查目录结构..." ;;
				files_dir_not_found) echo "目录不存在，创建: $2" ;;
		...
		ja)
			case "$key" in
				...
				electron_clean_install) echo "クリーンアップ後に再インストールすることをお勧めします：" ;;
				electron_clean_commands) echo "  rm -rf node_modules" ;;
				electron_clean_commands2) echo "  pnpm install" ;;
				...
				app_starting_dev) echo "Electron アプリケーションを起動中（開発モード）..." ;;

				# 文件检查相关
				files_checking) echo "必要なファイルを確認中..." ;;
				files_not_found) echo "ファイルが見つかりません: $2" ;;
				files_check_success) echo "必要なファイルの確認が完了しました" ;;
				files_setting_permissions) echo "スクリプト実行権限を設定中..." ;;
				files_checking_dirs) echo "ディレクトリ構造を確認中..." ;;
				files_dir_not_found) echo "ディレクトリが存在しません。作成します: $2" ;;
		...
		en|*)
			case "$key" in
				...
				electron_clean_install) echo "Suggest clean and reinstall:" ;;
				electron_clean_commands) echo "  rm -rf node_modules" ;;
				electron_clean_commands2) echo "  pnpm install" ;;
				...
				app_starting_dev) echo "Starting Electron application (development mode)..." ;;
				...
				files_setting_permissions) echo "Setting script execution permissions..." ;;
				files_checking_dirs) echo "Checking directory structure..." ;;
				files_dir_not_found) echo "Directory does not exist, creating: $2" ;;
