...
- <img src="../imgs/svg/system/protect.svg" alt="状态保护" style="height: 14px; width: 14px; vertical-align: middle; margin-right: 4px; display: inline-block;"> **状态保护图标** - 长按3秒可切换保护状态。保护后，自动读写、托盘模式和防止休眠功能将被禁用，防止误操作。图标保护后显示为绿色并带有脉冲动画。
...
对于只读状态的设备，可以点击"配置为可读写"按钮将其挂载为读写模式。此操作需要管理员权限，系统会弹出密码输入对话框。

**注意事项：**

- 挂载操作需要管理员权限，请准备好您的系统密码
- 如果挂载失败，可能的原因包括：
  - 设备文件系统处于脏状态（如果该 NTFS 设备之前在 Windows 电脑上使用过，且 Windows 启用了快速启动功能，请将设备插回 Windows 电脑并完全关闭后再试）
  - 设备被其他程序占用
  - 系统权限问题
- 挂载后请安全弹出设备，避免数据丢失
...
- **系统依赖**：确保已安装所有系统依赖（Xcode Command Line Tools、Homebrew、MacFUSE、ntfs-3g）
- **管理员权限**：确保输入的管理员密码正确
- **文件系统状态**：如果该 NTFS 设备之前在 Windows 电脑上使用过，且 Windows 启用了快速启动功能，文件系统可能处于脏状态。请将设备插回 Windows 电脑并完全关闭（而非休眠），然后重新尝试挂载
- **设备占用**：检查设备是否有其他程序正在使用，关闭相关程序后重试
- **操作超时**：如果操作超时，应用会自动取消操作以防止卡死。请检查上述原因后重试
...
操作日志默认关闭，需要手动启用。在"操作日志"标签页中勾选"启用操作日志"复选框即可开启。启用后，应用会记录所有操作日志。关闭后，将不再记录新的日志，但已记录的日志仍会保留。

**日志存储：**

日志以 JSON 格式保存在文件系统中，存储位置为：

```
~/Library/Application Support/Nigate/logs.json
```

日志文件采用格式化的 JSON 格式，可以直接用文本编辑器打开查看。卸载应用时，日志文件不会自动删除，便于后续查看或备份。

**日志限制：**

为了确保应用性能和稳定性，日志系统设置了以下限制：

- **保留期限**：日志默认最长保存 30 天，超过 30 天的日志会自动清理
- **记录数量限制**：最多保留 500 条日志记录，超过限制时会自动删除最旧的记录，保留最新的 500 条
- **文件大小限制**：日志文件最大为 500KB，超过限制时会自动删除旧记录以保持文件大小在限制内
- **显示数量限制**：界面中最多显示 300 条日志（最新的 300 条），以提高渲染性能

**日志管理：**

- **清空日志**：点击"清空"按钮可以清除所有日志记录
- **导出日志**：点击"导出"按钮可以将日志导出为文本文件，便于备份或分享
