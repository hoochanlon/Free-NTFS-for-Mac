...
- **自動讀寫圖示** - 啟用後，新插入的 NTFS 裝置會自動掛載為讀寫模式。圖示啟用時顯示為藍色。
- **托盤模式圖示** - 啟用後，關閉視窗時應用程式會最小化到系統托盤而不是退出。圖示啟用時顯示為紫色。
- **防止休眠圖示** - 啟用後，系統將禁止進入休眠狀態，確保裝置持續可用。圖示啟用時顯示為咖啡色。
- **關於圖示** - 開啟關於視窗，查看應用程式資訊和專案連結。
- **退出圖示** - 退出應用程式。

### 標籤頁圖示

- **日誌圖示** - 切換到「操作日誌」標籤頁，查看所有操作的記錄。

### 裝置管理圖示

- **重新整理圖示** - 重新整理裝置列表，重新偵測已連接的 NTFS 裝置。
