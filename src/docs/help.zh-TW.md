...
對於只讀狀態的設備，可以點擊"讀寫"按鈕將其掛載為讀寫模式。此操作需要管理員權限，系統會彈出密碼輸入對話框。
...
如果遇到 `Error opening '/dev/diskXsX': Resource busy` 錯誤，這通常是因為操作被非法強制中斷導致設備仍被佔用。解決方法：

1. **確認設備路徑**：使用 `diskutil list` 命令確認對應的磁碟設備（如 `/dev/disk5s1`）

2. **卸載設備**：
   ```bash
   sudo diskutil unmount /dev/disk5s1
   ```
   注意：將 `/dev/disk5s1` 替換為你實際的設備路徑
